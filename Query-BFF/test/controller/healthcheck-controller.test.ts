...
describe("Check class \'HealthCheckController\' ", () => {
    test('service and all dependencies are up', async () => {
        ...
        mock.onGet(process.env.QUERY_SERVICE + '/health').reply(200, response)
        ...
        expect(res.send).toHaveBeenCalledWith({
            "status": "UP",
            "components": [{"query-service": {"status": "UP"}}]
        });
    });
    ...
    test('one or more service dependencies are down', async () => {
        ...
        mock.onGet(process.env.QUERY_SERVICE + '/health').reply(503, downresponse)
        ...
        expect(res.send).toHaveBeenCalledWith({
            "status": "DOWN",
            "components": [{"query-service": {"status": "DOWN"}}]
        });
    });
    ...
    test('network error. no connection to service dependencies', async () => {
        ...
        let mock = new MockAdapter(axios);
        mock.onGet(process.env.QUERY_SERVICE + '/health').timeout()
        ...
        expect(res.send).toHaveBeenCalledWith({
            "status": "DOWN",
            "components": [{"query-service": {"status": "ECONNABORTED"}}]
        });
    });
    ...
    test('catch method error', async () => {
        ...
        let mock = new MockAdapter(axios);
        mock.onGet(process.env.QUERY_SERVICE + '/health').networkError();
