...
const options = {
    swaggerDefinition: {
        openapi: '3.0.1',
        info: {
            title: 'Messaging Hub Query BFF',
            description: "An externally-exposed, client_credential-permissioned API designed for use by external systems to manage EPCIS queries",
            version: '1.0.0'
        },
        components: {
            securitySchemes: {
...
export class RouterController {
    ...
    private initControllers = (controllers: IRoutingController[]): void => {
        controllers.forEach((controller: IRoutingController) => {
            this.service.use('/queries', controller.router);
        });
    };
    ...
    public listen() {
        this.service.listen(this.port, () => {
            logger.info(`Query BFF started on PORT : [ ${this.port} ]`);
        })
    }
}