...
export class HealthCheckController {
    ...
    getHealth = async (req: express.Request, res: express.Response): Promise<boolean> => {
        ...
        let up = true;
        try {
            await axios.get(process.env.QUERY_SERVICE + '/health')
                .then((response) => {
                    componentstatus.push({"query-service": response.data});
                }, (error) => {
                    up = false;
                    if (error.code) {
                        componentstatus.push({"query-service": {status: error.code}});
                    } else {
                        componentstatus.push({"query-service": error.response.data});
                    }
                });
