...
const options = {
    swaggerDefinition: {
        openapi: '3.0.1',
        info: {
            title: 'Messaging Hub Administration BFF',
            description: "An externally-exposed, user-permissioned API designed for use by the Messaging Hub Administration UI",
            version: '1.0.0'
        },
        components: {
            securitySchemes: {
...
export class RouterController {
    ...
    private initMiddleware = (): void => {
        ...
        this.bffService.use(cookieParser());
    ...
    private initControllers = (controllers: IRoutingController[]): void => {
        controllers.forEach((controller: IRoutingController) => {
            this.bffService.use('/admin', controller.router);
        });
    };
    ...
    public listen() {
        this.bffService.listen(this.port, () => {
            logger.info(`Admin BFF Service started on PORT : [ ${this.port} ]`);
        })
    }
}