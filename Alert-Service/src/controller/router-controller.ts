 ...
 * Developed by IBM in partnership with Merck
...
if( !process.env.ALERT_SERVICE_LOCAL_PORT ){
...
const options = {
    definition: {
        openapi: '3.0.0',
        info: {
            title: 'Messaging Hub Alert Microservice',
...
export class RouterController {
    ...
    public listen() {
        this.service.listen(this.port, () => {
            logger.info(`Alert Service started on PORT : [ ${this.port} ]`);
        })
    }
}