...
const options = {
    swaggerDefinition: {
        openapi: '3.0.1',
        info: {
            title: 'Messaging Hub Event BFF',
            description: "An externally-exposed, client_credential-permissioned API designed for use by external systems to manage EPCIS events",
            version: '1.0.0'
        },
        components: {
            securitySchemes: {
...
export class RouterController {
    ...
    private initControllers = (controllers: IRoutingController[]): void => {
        controllers.forEach((controller: IRoutingController) => {
            this.service.use('/events', controller.router);
        });
    };
    ...
    public listen() {
        this.service.listen(this.port, () => {
            logger.info(`Event BFF started on PORT : [ ${this.port} ]`);
        })
    }
}