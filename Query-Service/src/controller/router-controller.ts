...
if (!process.env.QUERY_SERVICE_LOCAL_PORT) {
...
const options = {
    definition: {
        openapi: '3.0.0',
        info: {
            title: 'Messaging Hub Query Microservice',
...
export class RouterController {
    ...
    private initMiddleware = (): void => {
        ...
        this.service.use(function (err: any, req: any, res: any, next: any) {
            //logger.error(JSON.stringify(err));
            if ("entity.too.large" === err.type) {
                res.status(413).send({
                    success: false,
                    message: err.message + ". Maximum allowable size " + (err.limit / 1024 / 1024) + " MB"
                });
            } else {
    ...
    public listen() {
        this.service.listen(this.port, () => {
            logger.info(`Query Service started on PORT : [ ${this.port} ]`);
        })
    }
}