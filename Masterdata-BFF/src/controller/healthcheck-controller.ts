...
export class HealthCheckController {
    ...
    getHealth = async (req: express.Request, res: express.Response): Promise<boolean> => {
        ...
        let up = true;
        try {
            await axios.get(process.env.MASTERDATA_SERVICE + '/health')
                .then((response) => {
                    componentstatus.push({"masterdata-service": response.data});
                }, (error) => {
                    up = false;
                    if (error.code) {
                        componentstatus.push({"masterdata-service": {status: error.code}});
                    } else {
                        componentstatus.push({"masterdata-service": error.response.data});
                    }
                });
